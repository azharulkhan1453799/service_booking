...
$result = $conn->query("SELECT * FROM services $where LIMIT $start, $limit");
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>All Services - ServiceBook</title>
...
<body class="bg-gradient-to-br from-gray-100 to-white text-gray-900">
  ...
  <nav class="bg-white shadow-lg sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        <a href="index.php" class="text-3xl font-extrabold text-indigo-700 tracking-wide">ServiceBook</a>

        <!-- Centered Search -->
        <form method="GET" action="services.php" class="hidden md:flex items-center gap-2 w-full max-w-md justify-center">
        ...
        <div class="hidden md:flex space-x-6 text-lg items-center">
          ...
          <a href="services.php" class="text-indigo-700 font-semibold border-b-2 border-indigo-700">Services</a>
  ...
  <div class="max-w-7xl mx-auto px-4 py-14">
    <h2 class="text-4xl font-bold text-center text-gray-800 mb-12 tracking-wide">🌟 Explore Our Premium Services</h2>

    <?php if ($totalRows == 0): ?>
      <p class="text-center text-gray-600">No services found matching "<strong><?= htmlspecialchars($search); ?></strong>".</p>
    <?php else: ?>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        <?php while($row = $result->fetch_assoc()): ?>
        <div class="bg-white border border-gray-200 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
          ...
          <div class="p-6 space-y-3">
            ...
            <p class="text-2xl text-indigo-700 font-bold">₹<?= $row['price']; ?></p>
    ...
    <div class="flex justify-center items-center mt-12 space-x-2">
      <?php if ($page > 1): ?>
        <a href="?search=<?= urlencode($search); ?>&page=<?= $page - 1 ?>" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg">&larr; Prev</a>
      <?php endif; ?>

      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <a href="?search=<?= urlencode($search); ?>&page=<?= $i ?>" class="px-4 py-2 <?= ($i == $page) ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800' ?> rounded-lg hover:bg-indigo-500 hover:text-white"><?= $i ?></a>
      <?php endfor; ?>

      <?php if ($page < $totalPages): ?>
        <a href="?search=<?= urlencode($search); ?>&page=<?= $page + 1 ?>" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg">Next &rarr;</a>
      <?php endif; ?>
    </div>
    <?php endif; ?>
  </div>
